...
public class ReportGenerator {
         ...
         */
        public static void generateHtmlReport(List<NetworkLogManager> managers, String outputPath,
                ...
                try (PrintWriter writer = new PrintWriter(new FileWriter(outputPath))) {
                        ...
                        for (Map.Entry<String, String> entry : analytics.entrySet()) {
                                writer.println("            <div style='margin-bottom: 10px;'><strong>" + entry.getKey()
                                                + ":</strong> "
                                                + entry.getValue() + "</div>");
                        }
                        writer.println("        </div>");
                        ...
                        writer.println(
                                        "            <div class='stat-card'><h3>Log Count</h3><p>"
                                                        + managers.get(0).getLogCount() + "</p></div>");
                        ...
                        for (NetworkLogManager manager : managers) {
                                writer.println("        <div class='complexity-box'>" + manager.getComplexityReport()
                                                + "</div>");
                        }
                        ...
                        for (LogEntry log : logs) {
                                ...
                                writer.println("                    <td>"
                                                ...
                                                + "</td>");
                                writer.println("                    <td>"
                                                + (log.getSourceIP() != null ? log.getSourceIP() : "Internal")
                                                + "</td>");
                                writer.println(
                                                "                    <td>"
                                                                + (log.getUser() != null ? log.getUser() : "System")
                                                                + "</td>");
                                writer.println("                    <td>" + log.getAction() + "</td>");
                                writer.println(
                                                "                    <td><span class='" + sevClass + "'>"
                                                                + log.getSeverity() + "</span></td>");
                                writer.println("                </tr>");
                        }
                        writer.println("            </tbody>");
                        writer.println("        </table>");
